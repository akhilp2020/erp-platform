...
export default function Page() {
  ...
  const [form, setForm] = useState<any>({ name: "", role: "", tenantId: "",  });
  const [loaded, setLoaded] = useState(!isNew);

  useEffect(() => {
    if (!isNew) {
      fetch(`/api/staff/${id}`).then(r => r.json()).then(setForm).then(() => setLoaded(true));
    }
  }, [id, isNew]);
  ...
  async function onSubmit(e: any) {
    ...
    if (isNew) {
      ...
      await fetch(`/api/staff/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
    }
    router.push('/staff');
  }
  ...
  return (
    <main className="p-6">
      <h1 className="text-xl font-semibold mb-4">Staff – {isNew ? 'New' : 'Edit'}</h1>
      <form onSubmit={onSubmit} className="grid gap-3 max-w-xl">
        ...
        <label className="grid gap-1">
          ...
          <input className="border rounded px-2 py-1" value={form['role'] ?? ''} onChange={e => setForm({ ...form, 'role': e.target.value })} />
        </label>
        <label className="grid gap-1">
